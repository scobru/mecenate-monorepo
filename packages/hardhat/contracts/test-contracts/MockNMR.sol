...
contract MockNMR is IERC20 {
  ...
  function transferFrom(
    ...
    address _to,
    uint256 _value
  ) public onlyPayloadSize(3) returns (bool ok) {
  ...
  function mint(uint256 _value) public returns (bool ok) {
    ...
    if (_totalSupply < _value) return false;
  ...
  function numeraiTransfer(address _to, uint256 _value) public onlyPayloadSize(2) returns (bool ok) {
    ...
    if (_balances[_to] < _value) return false;

    if (_allowances[_to][msg.sender] < _value) return false;
