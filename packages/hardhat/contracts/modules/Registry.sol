...
contract Registry is Ownable {
  ...
  function addFactory(address factory, bytes calldata extraData) external onlyOwner {
    ...
    require(factoryData.status == FactoryStatus.Unregistered, "factory already exists at the provided factory address");
  ...
  function retireFactory(address factory) external onlyOwner {
    ...
    require(factoryData.status == FactoryStatus.Registered, "factory is not currently registered");
  ...
  function getFactory(address factory)
    public
    view
    returns (
      FactoryStatus status,
  ...
  function getPaginatedFactories(uint256 startIndex, uint256 endIndex)
    external
    view
  ...
  function register(
    ...
    (
      FactoryStatus status,
      uint16 factoryID, // bytes memory extraData

    ) = getFactory(msg.sender);

    // ensure that the caller is a registered factory
    require(status == FactoryStatus.Registered, "factory in wrong status");

    uint256 instanceIndex = _instances.length;
    _instances.push(Instance({instance: instance, factoryID: factoryID, extraData: extraData}));
  ...
  function getInstanceData(uint256 index)
    external
    view
    returns (
