...
import "../features/MecenateMarket.sol";
...
contract MecenateOTC is MatchingEvents, MecenateMarket {
    struct sortInfo {
    ...
    function setMinSell(
        ...
        require(
            address(pay_gem) != dustToken,
            "Can't set dust for the dustToken"
        );

        uint256 dust = PriceOracleLike(priceOracle).getPriceFor(
            dustToken,
            address(pay_gem),
            dustLimit
        );
