...
contract MecenateUsersCentral {
  ...
  mapping(address => Structures.UserCentral) private _metadata;

  event UserRegistered(address indexed user, Structures.UserCentral data);
  ...
  function getUserData(address user) public view returns (Structures.UserCentral memory data) {
    data = _metadata[user];
  }
