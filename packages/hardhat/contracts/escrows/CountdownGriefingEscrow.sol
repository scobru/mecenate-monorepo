...
contract CountdownGriefingEscrow is Countdown, Staking, EventMetadata, Operated, Template {
  ...
  function initialize(
    ...
    if (agreementParams.length != 0) {
      (uint256 ratio, Griefing.RatioType ratioType, uint256 agreementCountdown) = abi.decode(
        agreementParams,
        (uint256, Griefing.RatioType, uint256)
      );
  ...
  function finalize() public {
    ...
    if (totalStake > 0) {
      require(IERC20(BurnNMR.getTokenAddress()).approve(agreement, totalStake), "token approval failed");
  ...
  function cancel() public {
    // restrict access control
    require(
      isSeller(msg.sender) || isBuyer(msg.sender) || Operated.isOperator(msg.sender),
      "only seller or buyer or operator"
    );
    // restrict state machine
  ...
  /// @return paymentAmount uint128 paymentAmount set in initialization
  /// @return stakeAmount uint128 stakeAmount set in initialization
  /// @return ratio uint120 ratio used for initialization of agreement on completion
  /// @return ratioType Griefing.RatioType ratioType used for initialization of agreement on completion
  /// @return countdownLength uint128 countdownLength used for initialization of agreement on completion
  function getData()
    public
    view
    returns (
  ...
  /// @return status EscrowStatus status from of the following states:
