...
const index_1 = __importDefault(require("../../crypto-ipfs/index"));
...
const schemaEncoder = new eas_sdk_1.SchemaEncoder("bool verified ,address feed, bytes post,");
...
class MecenateClient {
    ...
    async createPostData(RawData) {
        try {
            const symmetricKey = index_1.default.crypto.symmetric.generateKey();
            const encryptedFile = index_1.default.crypto.symmetric.encryptMessage(symmetricKey, RawData);
            const symmetricKeyHash = await index_1.default.multihash({
                input: symmetricKey,
                inputType: "raw",
                outputType: "hex",
            });
            // datahash sha256(rawdata)
            const dataHash = await index_1.default.multihash({
                input: RawData,
                inputType: "raw",
                outputType: "hex",
            });
            ...
            const encryptedDataHash = await index_1.default.multihash({
            ...
            const proofHash58 = await index_1.default.multihash({
    ...
    async createPost(feedAddress, postRawData, buyer, postType, postDuration, buyerPayment, postStake, tokenId, funder, seller, useStake) {
        ...
        const proofOfHashEncode = await index_1.default.multihash({
    ...
    async submitData(feedAddress, symmetricKey, secretKey) {
        ...
        const proofHash58Digest = await index_1.default.multihash({
            input: proofHash58,
            inputType: "b58",
            outputType: "digest",
        });
    ...
    async retrievePost(feedAddress, secretKey) {
        ...
        const decodeHash = await index_1.default.multihash({
            input: encryptedKey,
            inputType: "sha2-256",
            outputType: "b58",
        });
        ...
        const _decodeHash = await index_1.default.multihash({
        ...
        const decryptFile = index_1.default.crypto.symmetric.decryptMessage(decrypted, response_Encrypteddatahash_JSON.encryptedData);
        if (decryptFile) {
            // wait 10 seconds
    ...
    async revealPost(feedAddress, symmetricKey, postRawData) {
        const symKeyHash = await index_1.default.multihash({
            input: JSON.stringify({ symmetricKey: symmetricKey }),
            inputType: "raw",
            outputType: "b58",
        });
        const rawDataHash = await index_1.default.multihash({
