...
async function multihashFrom(input, inputType) {
    ...
    if (inputType === "raw") {
        ...
        throw new Error(`Invalid inputType: ${inputType} should be one of [${inputTypes}]`);
    }
...
async function multihashTo(contentid, outputType) {
    ...
    if (outputType === "prefix") {
        ...
        throw new Error(`Invalid outputType: ${outputType} should be one of [${outputTypes}]`);
    }
}
const MecenateHelper = {
    multihash: async ({ input, inputType, outputType }) => multihashTo(await multihashFrom(input, inputType), outputType),
    constants: {
        RATIO_TYPES: {
            NaN: 0,
            Inf: 1,
