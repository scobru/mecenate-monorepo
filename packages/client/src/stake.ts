...
async function takeStake(_feedAddr: string, _stake: string) {
  await connectWallet();
  ...
  const tx = await MecenateFeed.takeStake(parseEther(_stake));
  if (tx) {
    console.log("Unstake Done!");
  }
}

async function takeFullStake(_feedAddr: string) {
  await connectWallet();
  // wait 2 seconds to connect to wallet
  await new Promise((r) => setTimeout(r, 2000));
  ...
  const tx = await MecenateFeed.takeFullStake();
  if (tx) {
    console.log("Unstake Done!");
  }
}

async function addStake(_feedAddr: string, _stake: string) {
  await connectWallet();
  // wait 2 seconds to connect to wallet
  await new Promise((r) => setTimeout(r, 2000));
  ...
  const tx = await MecenateFeed.addStake({
...
export { takeStake, takeFullStake, addStake };
