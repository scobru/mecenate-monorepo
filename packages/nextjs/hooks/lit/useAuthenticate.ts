...
export default function useAuthenticate(redirectUri?: string) {
  ...
  const authWithGoogle = useCallback(async (): Promise<void> => {
    ...
    try {
      const result: AuthMethod = (await authenticateWithGoogle(redirectUri as any)) as any;
  ...
  const authWithDiscord = useCallback(async (): Promise<void> => {
    ...
    try {
      const result: AuthMethod = (await authenticateWithDiscord(redirectUri as any)) as any;
      setAuthMethod(result);
    } catch (err) {
      setError(err);
    } finally {
  ...
  const authWithEthWallet = useCallback(
    async (connector: any): Promise<void> => {
      ...
      try {
        const { account, connector: activeConnector } = await connectAsync(connector);
        const signer = await activeConnector?.getSigner();
        const signMessage = async (message: string) => {
          const sig = await signer.signMessage(message);
          return sig;
        };
        const result: AuthMethod = await authenticateWithEthWallet(account, signMessage);
        setAuthMethod(result);
      } catch (err) {
        setError(err);
      } finally {
   ...
   */
  const authWithWebAuthn = useCallback(async (username?: string): Promise<void> => {
    setLoading(true);
    setError(undefined);
    setAuthMethod(undefined);
   ...
   */
  const authWithStytch = useCallback(async (accessToken: string, userId?: string): Promise<void> => {
    setLoading(true);
    setError(undefined);
    setAuthMethod(undefined);

    try {
      const result: AuthMethod = (await authenticateWithStytch(accessToken, userId)) as any;
      setAuthMethod(result);
    } catch (err) {
      setError(err);
    } finally {
