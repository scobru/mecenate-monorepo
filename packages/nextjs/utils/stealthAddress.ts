...
export default async function generateStealthAddress(
  ...
  const encryptedR = MecenateHelper.crypto.asymmetric.encrypt(
    r,
...
export async function verifyStealthAddress(
  ...
  const decryptedR = MecenateHelper.crypto.asymmetric.decrypt(
    encryptedR,
    nonce,
    ephemeralPubKey,
    receiverSecretKey,
...
export async function generateKeyPairFromSeed(provider: any, signer: any) {
  const baseMessage =
    'Sign this message to access your Mecenate account.\n\nOnly sign this message for a trusted client!';
