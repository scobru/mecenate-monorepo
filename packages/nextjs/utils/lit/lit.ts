...
import { AuthMethod, GetSessionSigsProps, IRelayPKP, ProviderOptions, SessionSigs } from "@lit-protocol/types";

export const DOMAIN = process.env.NEXT_PUBLIC_DOMAIN || "localhost";
export const ORIGIN =
  process.env.NEXT_PUBLIC_VERCEL_ENV === "production" ? `https://${DOMAIN}` : `http://${DOMAIN}:3000/wallet`;
 ...
 * Get auth method object from redirect
 */
export async function authenticateWithDiscord(redirectUri: string): Promise<AuthMethod | undefined> {
  const discordProvider = litAuthClient.initProvider<DiscordProvider>(ProviderType.Discord, { redirectUri });
...
export async function sendOTPCode(emailOrPhone: string) {
  const otpProvider = litAuthClient.initProvider<OtpProvider>(ProviderType.Otp, {
    userId: emailOrPhone,
 ...
 */
export async function authenticateWithOTP(code: string): Promise<AuthMethod | undefined> {
...
export async function mintPKP(authMethod: AuthMethod): Promise<IRelayPKP> {
  ...
  if (authMethod.authMethodType === AuthMethodType.WebAuthn) {
    ...
    // Verify registration and mint PKP through relay server
    txHash = await (provider as WebAuthnProvider).verifyAndMintPKPThroughRelayer(options);
  } else {
