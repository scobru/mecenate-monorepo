...
const Timestamps: NextPage = () => {
  ...
  const getPostResult = async () => {
    const contract = new Contract(feed, deployedContractFeed?.abi as ContractInterface, signer)
    try {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-10 bg-gradient-to-tl from-blue-950 to-slate-950 min-w-full min-h-screen">
      <div className="w-1/4">
        ...
        />
        <button
          className="btn btn-custom w-fit mb-4"
          onClick={getPostResult}
        >
          Get post timestamp
        </button>
        <div className="card  w-96 bg-gradient-to-br from-blue-950 to-slate-700 opacity-80 my-5 border-1">
          {result && result.postResult && result.creationTimeStamp && (
            <div className="card-body">
              ...
              <p>{result?.postResult == 0 ? "Nan" : result?.postResult == 1 ? "💫 Valid " : "Punished ❌"}</p>
              <h2 className="card-title">Start</h2>
              <p>{format(new Date(Number(result?.creationTimeStamp) * 1000), '🕜 dd/MM/yyyy HH:mm:ss')}</p>
              <h2 className="card-title">End</h2>
              <p>{format(new Date(Number(result?.endTimeStamp) * 1000), '🕝 dd/MM/yyyy HH:mm:ss')}</p>
            </div>
          )}
        </div>
      </div>
